#This function was adapted from https://github.com/hidasi/bivariatemaps
  ...
  if (plotLeg) {
    p <- ggplot(col.matrix.plot, aes(X, Y, fill = HEXCode)) +
      ...
      theme(aspect.ratio = 1,
            axis.title = element_text(size = 9, colour = "black",hjust = 0.5, 
                                      vjust = 1,margin = margin(t = 15)),
            axis.title.y = element_text(angle = 90, hjust = 0.5,margin = margin(r = 15))) +
 
      
      #geom_line(shape = 18, color = "white", width = 3, fill="white",size =3,aes(xmin=2, xmax = 1 , ymin=0, ymax = 3), size=1.5)+
      geom_vline(aes(xintercept = 1.5), col = "grey93",width = 3,size =3) +
      geom_hline(aes(yintercept = 1.5), col = "grey93",width = 3,size =3) +
     
      #geom_brace(aes(c(1,2), c(0.1, 0.3)), inherit.data=F,col = "black")+
      #eje y
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 2.3 , y=2.3, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 2 , y=2, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 1.7 , y=1.7, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 1.5 , y=1.5, yend = 1.5), size=1.5)+
      #geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 1.3 , y=1.3, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 1 , y=1, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 0.7 , y=0.7, yend = 1.5), size=1.5)+
      
      #eje x
      #geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=2.3, xend = 2.3 , y=1.5, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=2, xend = 2 , y=1.5, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.7, xend = 1.7 , y=1.5, yend = 1.5), size=1.5)+
      #geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.5, xend = 1.5 , y=1.5, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1.3, xend = 1.3 , y=1.5, yend = 1.5), size=1.5)+
      #geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=1, xend = 1 , y=1.5, yend = 1.5), size=1.5)+
      # geom_point(shape = 18, color = "white", width = 3, fill="white",size =3,aes(x=0.7, xend = 0.7 , y=1.5, yend = 1.5), size=1.5)+
      
      
      
      xlab(bquote(.(xlab) ~  symbol(""))) +
          ylab(bquote(.(ylab) ~  symbol("")))
  #p+ geom_brace(aes(x=c(1,2), y=c(0.1,0.4)), inherit.data=FALSE)+
    print(p)
    assign(
      x = "BivLegend",
      value = p,
...
bivariate.map <- function(rasterx, rastery, colourmatrix = col.matrix,
  ...
  #brks=quantile(present, probs = c(0, 0.50, 1),names = FALSE)
  #brks=as.vector(brks)
 brks <- with(tempx, classIntervals(quanx,
  ...
  #brksy <- c(minValue(rasterx), brks[2], maxValue(rasterx))
 brksy<-brks
